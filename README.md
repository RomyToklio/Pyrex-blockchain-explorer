...
Currently available Pyrexcoin blockchain explorers have several limitations which are of
special importance to privacy-oriented users:
 ...
 - do not support Pyrexcoin testnet nor stagenet networks,
 - have limited JSON API.


In this example, these limitations are addressed by development of
an Onion Pyrexcoin Blockchain Explorer. The example not only shows how to use
Pyrexcoin C++ libraries, but also demonstrates how to use:
...
i2p users (main Pyrexcoin network):
...
## Onion Pyrexcoin Blockchain Explorer features

The key features of the Onion Pyrexcoin Blockchain Explorer are:
...
Note: `devel` branch of the explorer follows `master` branch of the Pyrexcoin.

## Compilation on Ubuntu 16.04/18.04

##### Compile latest Pyrexcoin version (0.14.0.0)

Download and compile recent Pyrexcoin into your home folder:

```bash
# first install Pyrexcoin dependecines
sudo apt update
...
# download Pyrexcoin sourced for branch release-v0.13 
git clone --recursive -b release-v0.13 https://github.com/pyrex-coin/Pyrex
...
Once the Pyrexcoin is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/Pyrex
cd ~

# download the source code 
git clone https://github.com/pyrex-coin/Pyrex-blockchain-explorer.git

# enter the downloaded sourced code folder
cd Pyrex-blockchain-explorer
...
```bash
./xmrblocks -b /home/mwo/non-defult-pyrexcoin-location/lmdb/
```

Example output:

```bash
[mwo@arch Pyrex-blockchain-explorer]$ ./xmrblocks
...
```
xmrblocks, Onion Pyrexcoin Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Pyrexcoin total emission monitoring
                                        thread
...
alias xmrblocksmainnet='~/Pyrex-blockchain-explorer/build/xmrblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias xmrblockstestnet='~/Pyrex-blockchain-explorer/build/xmrblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:8081/ and http://139.162.32.245:8082/, respectively.

## Enable Pyrexcoin emission

Obtaining current Pyrexcoin emission amount is not straight forward. Thus, by default it is
...
```
Pyrexcoin emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Pyrexcoin daemon's  `print_coinbase_tx_sum` command.
...
```bash
# we use here official Pyrexcoin project's donation address as an example
...
Return raw json block data, as represented in Pyrexcoin.
...
Return raw json tx data, as represented in Pyrexcoin.
